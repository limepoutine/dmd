 ...
 */
public void inlineScanAlwaysInline(Module m, ErrorSink eSink)
{
    if (m.semanticRun != PASS.semantic3done)
        return;

    inlineScanModule(m, PASS.inline1, eSink);
}
...
{
    if (m.semanticRun != PASS.semantic3done && m.semanticRun != PASS.inline1)
        return;

    inlineScanModule(m, PASS.inline2, eSink);
}
...
public:
    ...
    override void visit(FuncDeclaration fd)
    {
        ...
        if (!fd.hasAlwaysInlines && pass == PASS.inline1)
            return;
...
{
    assert(pass == PASS.inline1 || pass == PASS.inline2);
...
Lno:
    if (fd.inlining == PINLINE.always && pass == PASS.inline1 &&
